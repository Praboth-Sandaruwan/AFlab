...
describe("Goals API", () => {
  ...
  it("should get all user goals of every user", async () => {
    const res = await request(app)
      .get("/api/goals/all")
      .set("Authorization", `Bearer ${token}`);
  ...
  it("should update any goal", async () => {
    const goalCheck = await Goal.findById(goalId);
    console.log(goalId);
    console.log("Goal Found:", goalCheck);

    const res = await request(app)
      .put(`/api/goals/any/${goalId}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ savedAmount: 1000, priority: 5, title: "Goal Title" });
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body.savedAmount).toBe(1000);
  });
  ...
  it("should delete any goal", async () => {
    const res = await request(app)
      .delete(`/api/goals/any/${goalId}`)
      .set("Authorization", `Bearer ${token}`);
